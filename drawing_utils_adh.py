...
def draw_detection(
  ...
  for keypoint in location.relative_keypoints:
    ...
    point_set.append(keypoint_px)  # 추가
 #-------------------------------------------------------------------------------->
  # 0--> 우측 눈, 1--> 좌측 눈, 2--> 코, 3--> 입, 4--> 우측 귀, 5--> 좌측 귀, 6--> 양측 눈
  '''
  keypoint=location.relative_keypoints[5]
  keypoint_px=_normalized_to_pixel_coordinates(keypoint.x, keypoint.y, image_cols, image_rows)
  cv2.circle(image, keypoint_px, keypoint_drawing_spec.circle_radius, keypoint_drawing_spec.color, keypoint_drawing_spec.thickness)
  '''
 #-------------------------------------------------------------------------------->   
  ...
  if flag==0 or flag==1: # 눈
    ...
    return eye_rect_start_point, eye_rect_end_point, point_set

  elif flag==2:  # 코
    relative_bounding_box = location.relative_bounding_box
    nose_rect_start_point = _normalized_to_pixel_coordinates(location.relative_keypoints[flag].x-relative_bounding_box.width*0.22, location.relative_keypoints[flag].y-relative_bounding_box.height*0.3, image_cols, image_rows)
    nose_rect_end_point = _normalized_to_pixel_coordinates(location.relative_keypoints[flag].x+relative_bounding_box.width*0.22, location.relative_keypoints[flag].y+relative_bounding_box.height*0.3, image_cols, image_rows)
    cv2.rectangle(image, nose_rect_start_point, nose_rect_end_point, bbox_drawing_spec.color, bbox_drawing_spec.thickness)
    print(f"x: {location.relative_keypoints[flag].x}, y: {location.relative_keypoints[flag].y}")
    return nose_rect_start_point, nose_rect_end_point, point_set
    ...
    print(f"x: {location.relative_keypoints[flag].x}, y: {location.relative_keypoints[flag].y}")
    return lips_rect_start_point, lips_rect_end_point, point_set
    ...
    print(f"x: {location.relative_keypoints[flag].x}, y: {location.relative_keypoints[flag].y}")
    return ear_rect_start_point, ear_rect_end_point, point_set

  elif flag==6:   # 양측 눈
    relative_bounding_box = location.relative_bounding_box
    leye_rect_start_point = _normalized_to_pixel_coordinates(location.relative_keypoints[0].x-relative_bounding_box.width*0.17, location.relative_keypoints[0].y-relative_bounding_box.height*0.15, image_cols, image_rows)
    reye_rect_end_point = _normalized_to_pixel_coordinates(location.relative_keypoints[1].x+relative_bounding_box.width*0.17, location.relative_keypoints[1].y+relative_bounding_box.height*0.1, image_cols, image_rows)
    cv2.rectangle(image, leye_rect_start_point, reye_rect_end_point, bbox_drawing_spec.color, bbox_drawing_spec.thickness)
    #print(f"x: {location.relative_keypoints[flag].x}, y: {location.relative_keypoints[flag].y}")
    return leye_rect_start_point, reye_rect_end_point, point_set
 ...
 #-------------------------------------------------------------------------------->
  ...
  if not location.HasField('relative_bounding_box'):
    return
  relative_bounding_box = location.relative_bounding_box
  rect_start_point = _normalized_to_pixel_coordinates(relative_bounding_box.xmin-relative_bounding_box.width*0.2, relative_bounding_box.ymin-relative_bounding_box.height*0.2, image_cols, image_rows) # 좌측 상단 지점 (기준: 좌측 상단 0,0)
  rect_end_point = _normalized_to_pixel_coordinates(relative_bounding_box.xmin + relative_bounding_box.width*1.2, relative_bounding_box.ymin + relative_bounding_box.height*1.2, image_cols,image_rows)

  cv2.rectangle(image, rect_start_point, rect_end_point, bbox_drawing_spec.color, bbox_drawing_spec.thickness)

  #-------------------------------------------------------------------------------->
  return rect_start_point, rect_end_point, point_set
  #-------------------------------------------------------------------------------->
